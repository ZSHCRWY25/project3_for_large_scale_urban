...
class reciprocal_vel_obs:
    ...
    def cal_vel(self, agent_state, nei_state_list=[], obs_cir_list=[], obs_line_list=[], mode = 'rvo'):
        
        agent_state, ns_list, oc_list, ol_list = self.preprocess(agent_state, nei_state_list, obs_cir_list, obs_line_list)
    ...
    def preprocess(self, agent_state, nei_state_list, obs_cir_list, obs_line_list):
        ...
        ns_list = list(filter(lambda x: 0 < reciprocal_vel_obs.distance(agent_state, x) <= self.nr, nei_state_list))
        oc_list = list(filter(lambda y: 0 < reciprocal_vel_obs.distance(agent_state, y) <= self.nr, obs_cir_list))
        ol_list = list(map(lambda z: reciprocal_vel_obs.segment_in_circle(agent_state[0], agent_state[1], self.nr, z), obs_line_list))
    ...
    def config_vo(self, agent_state, nei_state_list, obs_cir_list, obs_line_list, mode):
        ...
        vo_list1 = list(map(lambda x: self.config_vo_circle(agent_state, x, mode), nei_state_list))
        vo_list2 = list(map(lambda y: self.config_vo_circle(agent_state, y, 'vo'), obs_cir_list))
        vo_list3 = list(map(lambda z: self.config_vo_line(agent_state, z), obs_line_list))
    ...
    def vel_select(self, agent_state, vo_outside, vo_inside, nei_state_list, obs_cir_list, obs_line_list, vo_list):
        ...
        if (len(vo_outside) != 0):
            ...
            temp = min(vo_inside, key = lambda v: self.penalty(v, vel_des, agent_state, nei_state_list, obs_cir_list, obs_line_list, 1) )
    ...
    def penalty(self, vel, vel_des, agent_state, nei_state_list, obs_cir_list, obs_line_list, factor):
        ...
        for obs_seg in obs_line_list:
            tc = reciprocal_vel_obs.exp_collision_segment(obs_seg, agent_state[0], agent_state[1], vel[0], vel[1], agent_state[4])
            tc_list.append(tc)
