...
class ir_gym(env_base):
    def __init__(self, world_name, neighbors_region=5, neighbors_num=10, vxmax = 1.5, vymax = 1.5, vzmax = 1.5, env_train=True, acceler = 0.5, **kwargs):
    ...
    def cal_des_list(self):#计算机器人的全向移动（omni-directional）的目标速度列表。(改完)
        des_vel_list = [drone.cal_des_vel() for drone in self.Drone_list]#调用其 cal_des_vel() 方法以计算目标速度
        return des_vel_list


    def rvo_reward_list_cal(self, action_list, **kwargs):#计算机器人在执行一系列动作时的 RVO（Reciprocal Velocity Obstacles）奖励列表   （要不要加建筑物） 
        ts = self.components['drone'].total_states() # #获取机器人的状态状态

        rvo_reward_list = list(map(lambda drone_state, action: self.rvo_reward_cal(drone_state, ts, action, self.reward_parameter, **kwargs), ts[0], action_list))#对于每个动作，调用 rvo_reward_cal 方法计算奖励
    ...
    def obs_move_reward_list(self, action_list, **kwargs):#计算机器人执行一系列动作时的观测和奖励
        ...
        obs_reward_list = list(map(lambda drone, action: self.observation_reward(drone, ts, action, **kwargs), self.Drone_list, action_list))
        ##下面的没改
...
#       返回观测列表、奖励列表、完成标志列表和其他信息列表。
    ...
    def observation_reward(self, drone, odro_state_list, obs_list,action, **kwargs):#算机器人的观测和奖励。
       ...
       # 返回观测、奖励、完成标志和其他信息。
        ...
        if drone.arrive() and not drone.arrive_flag:
        ...
        if len(obs_vo_list) == 0:
            ...
            exter_obs = np.concatenate(obs_vo_list) # vo list外部观测
